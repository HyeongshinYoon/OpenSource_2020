...
def addBody():
    ...
    body_address = [f for f in listdir("./data/test/image/")]
    for i in range(len(body_address)):
        address = "./data/test/image/"+str(body_address[i])
        with open(address, 'rb') as imageFile:
            a = imageFile.read()
        storage.child(body_address[i]).put(a)
        db.child('bodys').push(body_address[i])
